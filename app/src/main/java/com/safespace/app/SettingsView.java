...
final class SettingsView extends FrameLayout {
    ...
    SettingsView(Activity activity, ScreenNavigator navigator) {
        ...
        addDivider(card);
        card.addView(settingRow("ⓘ", "About", "›", () -> toast("Safe Space 1.3.0")));
    ...
    private void selectAppearance(int index) {
        ...
        if (index == 1) {
            postDelayed(() -> navigator.openScreen(20), 120L);
        } else {
            toast(index == 0 ? "Light appearance selected" : "Following system appearance");
        }
    }
